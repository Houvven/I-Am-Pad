...
extract "$ZIPFILE" 'impad.dex'      "$MODPATH"
...
if [ "$ARCH" = "x86" ] || [ "$ARCH" = "x64" ]; then
  if [ "$HAS32BIT" = true ]; then
    ...
    extract "$ZIPFILE" "lib/x86/libdexkit.so" "$MODPATH/lib/" true
    extract "$ZIPFILE" "lib/x86/libpine.so" "$MODPATH/lib/" true
  else
    extract "$ZIPFILE" "lib/x86_64/libdexkit.so" "$MODPATH/lib/" true
    extract "$ZIPFILE" "lib/x86_64/libpine.so" "$MODPATH/lib/" true
  fi
  ...
  if [ "$HAS32BIT" = true ]; then
    ...
    extract "$ZIPFILE" "lib/armeabi-v7a/libdexkit.so" "$MODPATH/lib/" true
    extract "$ZIPFILE" "lib/armeabi-v7a/libpine.so" "$MODPATH/lib/" true
  else
      extract "$ZIPFILE" "lib/arm64-v8a/libdexkit.so" "$MODPATH/lib/" true
      extract "$ZIPFILE" "lib/arm64-v8a/libpine.so" "$MODPATH/lib/" true
  fi
